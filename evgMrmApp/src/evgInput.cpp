...
evgInput::evgInput(const std::string &name, const epicsUInt32 num,
    ...
    : mrf::ObjectInst<evgInput>(name), m_num(num), m_type(type), m_pInReg(pInReg)
{
...
void evgInput::setDbusMap(epicsUInt16 dbus, bool ena)
{
    if (dbus > 7)
...
void evgInput::setTrigEvtMap(epicsUInt16 trigEvt, bool ena)
{
    if (trigEvt > 7)
